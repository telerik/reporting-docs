...
- Hard rules to enforce while editing (non-exhaustive — defer to upstream for the full list):
  ...
  - **Images audit (per article).** Treat images as first-class content, not decoration. For every image reference in the article:
    ...
    3. **Verify and improve alt text.** Load the [alt-text-writer](../skills/alt-text-writer/SKILL.md) skill and evaluate every image's alt text against its rules. Rewrite alt text that is empty, uses the file name, is too short (< 4 words), lacks a verb, or does not follow the skill's sentence structure. Do not invent content the image doesn't show.

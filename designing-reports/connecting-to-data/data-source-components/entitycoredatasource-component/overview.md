...
- Your Entity Framework Core (EF Core) assembly must be allowed and accessible by the Report Designer, along with all its dependencies. For the custom EF Core assembly, you need to [extend the report designer to recognize custom assemblies](slug:telerikreporting/designing-reports/report-designer-tools/desktop-designers/standalone-report-designer/configuration/extending-report-designer). Its dependencies, though, must be copied manually to the report designer's folder. You can take them from the `bin` folder of the corresponding project after building it.

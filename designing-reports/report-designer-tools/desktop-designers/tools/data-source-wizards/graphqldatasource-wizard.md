...
1. **Configure data retrieval**: Setup the properties of the HTTP request that will retrieve data from the GraphQL service:
...
1. **Configure GraphQL Query**: Allows you to define the actual GraphQL query that will be sent to the service. You can write a static query or use inline parameters to dynamically inject values into the query string.
...
1. **2-Step Authentication**
   ...
   | Token Path  | This is a regular expression that allows to retrieve the authentication or session key from the response received via the Login URL. For example, when the Login URL returns a JSON response containing the authentication token in the form: _{"access_token":"example-access-token","token_type":"bearer","expires_in":1799,"userName":"demouser",".issued":"Tue, 15 May 2018 08:42:32 GMT",".expires":"Tue, 15 May 2018 09:12:32 GMT"}_ the token path regular expression to retrieve the token would be: _(?:"access_token":")(._?)(?:")\* |
...
1. **2-Step Authentication Parameters**: The parameters of the HTTP request to be applied. The supported types are Query, Header, Cookie, and Inline. The inline parameters are parameters that are used to replace parameter tokens (@param1) in the URL and the Body fields with an expression result or static value.

1. **Preview data source results**: Allows you to preview the result from the configurations you have applied so far.

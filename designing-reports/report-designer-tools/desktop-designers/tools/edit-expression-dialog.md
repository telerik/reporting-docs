...
To use the `Edit Expression` dialog, either type a [report expression](slug:telerikreporting/designing-reports/connecting-to-data/expressions/overview) in directly or double-click the list of elements in the bottom middle list to add them to the expression. In the figure below, an expression with a database field multiplied by a coefficient provides an extended price value.
...
For a list of the supported expression elements, see [the item binding expressions guide](slug:telerikreporting/designing-reports/connecting-to-data/expressions/using-expressions/overview).

...
1. Click the **AI Assist** button at the bottom-right corner of the report area.
...
AI Assist chooses between a Create flow, an Edit flow, and a Question flow based on the message you send and the current selection on the design surface when you open the panel.
...
1. Open the [AI Assist](#opening-ai-assist).
...
The **AI Assist** is built on an agentic loop powered by `Microsoft.Extensions.AI` function tools. The agent has access to a small, focused tool surface that lets it inspect the current report and craft a valid item definition without inventing schema or data:
...
| `GetSkill` | Returns cross-cutting authoring skills that cover concerns such as expressions, conditional formatting, bindings, aggregates, sorting, and filtering. The agent loads only the skills relevant to the user's intent. |
...
After the agent crafts a candidate item, a validator deserializes the JSON against the report model and returns any errors in natural language. The agent revises the JSON and retries until the item is valid or a configured retry limit is reached.
...
## Configuring AI Assist in the Host Application

AI Assist runs as a SignalR-backed service in the application that hosts the Web Report Designer. The host application controls which language model the agent uses, when chat sessions expire, and which users may invoke the feature.

To enable AI Assist, register the agent services in `Startup.ConfigureServices` and map the agent endpoint in `Startup.Configure`. The following snippet shows the typical wiring with a custom `IChatClient` factory (for example, Azure OpenAI) and explicit option values:

```CSharp
public void ConfigureServices(IServiceCollection services)
{
    // ... existing Reporting REST and Web Report Designer registrations ...
...
{
    var apiKey = configuration["ai-client-credentials"];
    return new AzureOpenAIClient(
            new Uri("https://your-azure-openai-endpoint.cognitiveservices.azure.com"),
...
public void Configure(IApplicationBuilder app)
{
    ...
    app.UseEndpoints(endpoints =>
    {
        ...
        endpoints.UseAIAgentServices();
    });
}
```

`UseAIAgentServices` maps the agent SignalR hub at `/wrd-ai-agent`. The Web Report Designer client connects to this endpoint when the **AI Assist** button is invoked. If the host removes this registration, the **AI Assist** button does not appear in the designer.

As an alternative to passing values directly, call the `AddAIAgentServices(IConfiguration)` overload to read the options from an `AIAgent` section in `appsettings.json`:
...
| Option | Description |
|--------|-------------|
| `SessionIdleTimeoutMinutes` | Idle timeout, in minutes, for an AI Assist chat session. When omitted, the default `IDistributedCache` expiration applies. |
...
To restrict who can invoke AI Assist, gate the `Commands_AIAgent_Use` permission in your `Telerik.WebReportDesigner.Services.Models.Permission` authorization configuration. Users who lack this permission do not see the **AI Assist** entry point.

> note AI Assist is a separate NuGet package, `Telerik.WebReportDesigner.AI`, which the host application must reference in addition to the standard Web Report Designer packages.

## Security, Privacy, and Limits

The **AI Assist** sends the user prompt, the relevant JSON Schema, and the necessary report metadata to the configured language model. Live data values are not sent unless the chat explicitly references them.
...
- Administrators control availability through host-level service registration and the `Commands_AIAgent_Use` permission, as described in [Configuring AI Assist in the Host Application](#configuring-ai-assist-in-the-host-application).

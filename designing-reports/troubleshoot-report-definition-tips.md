...
description: "Learn how to troubleshoot report structure problems when working with Telerik Reporting."
...
*Increase the Report Page Width in one of the following manners:
	+ Change the Report `PageSettings` > `PaperKind` to a bigger one that will accommodate the content;
	+ Inrease the Report `PageSettings` > `PaperSize` > `Width` for `Custom` paper kind;
	+ Decrease the Report `PageSettings` > `Margins` > `Left` and/or `Right`.

Check for [Binding]({%slug telerikreporting/designing-reports/connecting-to-data/expressions/using-expressions/bindings%}) and [Conditional Formatting]({%slug telerikreporting/designing-reports/connecting-to-data/expressions/using-expressions/conditional-formatting%}) that may be altering the report layout only at runtime.

## Troubleshooting Report Layout

Use different __Styles__, for example, `Background Color` and `Borders` to disctinguish between report sections and items and identify which one is responsible for the observed inconsistency in the report layout.

### Unexpected Page Breaks

* Page Break Properties:
	Inspect the [Report Sections]({%slug report_structure_groups_sections%}#report-sections) for `PageBreak` property and [Report Groups]({%slug telerikreporting/designing-reports/connecting-to-data/data-items/grouping-data/how-to-add-groups-to-report%}) and [Table Groups]({%slug telerikreporting/designing-reports/connecting-to-data/data-items/grouping-data/how-to-add-groups-to-table-item-and-crosstab-item%}) for the `GroupPageBreak` property settings.
...
* Keep Together Settings:
	...
	When the property value is `True`, the Reporting Engine tries to render the entire item/section/group content on the same page. If it doesn't fit, it attempts to move the whole content on a new page. When the content fits on the new page, the previous page may remaing partly blank. If this is not the desired outcome, ensure the corresponding report items/sections/groups have their keep together values set to false.

* Horizontal Pagination:
	Generally, the [Report items]({%slug telerikreporting/designing-reports/report-structure/overview%}) grow only vertically to accommodate larger content. Only [Crosstabs and Tables/Lists with column gouping]({%slug table_template_items%}), _rotated Tables_ and [SubReports]({%slug telerikreporting/designing-reports/report-structure/subreport%}) may grow horizontally. In case on unexpected horizontal page breaks, inspect the items runtime content.
...
The [Visual Studio Report Designer for .NET Framework]({%slug telerikreporting/designing-reports/report-designer-tools/desktop-designers/visual-studio-report-designer/overview%}) generates automatically the code for the CS/VB reports and stores it in the `InitializeComponent` method of the file _ReportName_.designer.cs/_ReportName_.Designer.vb. Check the file for errors it the report cannot be opened in the designer.

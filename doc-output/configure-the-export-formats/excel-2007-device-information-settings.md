...
|UseSvgImages|Boolean|A flag specifying whether to embed SVG images for Graph, Gauge, and Barcode report items using the Office 2019 SVGBlip extension. When enabled, the exported items use resolution-independent vector graphics instead of raster images. The default value is __false__.|
|IncludeRasterFallback|Boolean|When `UseSvgImages` is enabled, specifies whether to include a raster fallback image alongside the embedded SVG. The fallback ensures compatibility with Office versions older than 2019 that do not support the SVGBlip extension. The default value is __true__.|

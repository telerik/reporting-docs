...
| UseSvgImages          | Boolean  | A flag specifying whether the Skia graphics engine embeds SVG images for Graph, Gauge, Barcode, and Cross-Section report items using the Office 2019 SVGBlip extension. When enabled, the exported items use resolution-independent vector graphics. When disabled, the Skia engine inserts raster images instead. This setting does not affect the GDI graphics engine, which uses EMF images. The default value is **true**. Shape items are not affected because the Word rendering extension already uses native DrawingML vector paths for shapes. |

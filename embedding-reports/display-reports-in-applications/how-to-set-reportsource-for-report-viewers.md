...
* __Set the Report Source through the Visual Studio Designer__
	...
	1. Select the type of report source you would use and specify a report. For this example, we would use a __UriReportSource__, click the 'URL or file' option, and select the report that would be shown in the viewer. If you have to specify parameters for the report, continue with the next step.
...
These Report Viewers do not have direct access to the Telerik Reporting Engine. Reports are processed and rendered on a remote machine where Telerik Report Server is running, and the content is delivered to the client by __Telerik Report Server's REST Service__.

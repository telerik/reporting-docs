...
1. Add the following xaml files, required for the implicit styling of the report viewer:
   - _System.Windows.xaml_
   - _Telerik.Windows.Controls.xaml_
   - _Telerik.Windows.Controls.Input.xaml_
   - _Telerik.Windows.Controls.Navigation.xaml_
   - _Telerik.ReportViewer.Silverlight.xaml_ The Telerik UI for Silverlight xaml files are located in _C:\Program Files (x86)\Progress\Reporting {{site.suiteversion}}\Silverlight\Themes_.
...
1. Add the ReportViewer to the MainPage.xaml from the **ToolBox** through **Visual Studio** or **Expression Blend**. You should skip points #5, #6 and #7 if you have done that, as they describe how to register the report viewer manually.
...
1. Next we need to set the **ReportServiceUri** and **Report** properties of the viewer.

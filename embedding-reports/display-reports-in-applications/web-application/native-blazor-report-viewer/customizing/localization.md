...
1. Enable localization in the Blazor app from inside `Program.cs`(or `Startup.cs` if .NET 5 or older is used):
	...
	{{source=CodeSnippets\BlazorNative\Docs\Program.cs region=ProgramAddLocalization}}

1. Add `.resx` files to the `~/Resources` folder
...
1. (_Optional_) Now to test that the localization is working, you may change the culture in the component with the report viewer through the following code:
	...
	{{source=CodeSnippets\BlazorNative\Docs\ReportViewers\NativeBlazorViewerWithRest.razor region=BlazorComponentSetCulture}}

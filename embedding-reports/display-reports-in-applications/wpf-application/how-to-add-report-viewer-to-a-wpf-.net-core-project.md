---
title: Integrating in .NET Core
page_title: Using the WPF Report Viewer in .NET Core
description: "Learn how to start using the Telerik Reporting WPF Report Viewer in .NET Core applications with this tutorial."
...
# Integrating the WPF Report Viewer in .NET Core

This article explains the steps needed to integrate the WPF report viewer to a .NET Core 3.1 and .NET 5+ aprojects.
...
### Steps to add report viewer to a WPF .NET Core 3.1 or .NET 5+ project
...
1. Add the following assembly references (available in Telerik Reporting installation `Bin` directory) or NuGet package references (available in [Telerik private NuGet repository]({%slug telerikreporting/using-reports-in-applications/how-to-add-the-telerik-private-nuget-feed-to-visual-studio%})):
	...
	The `Telerik.ReportViewer.Wpf` and `Telerik.ReportViewer.Wpf.Themes` assemblies can be found in `%programfiles(x86)%\Progress\Telerik Reporting {{site.suiteversion}}\Bin`. Select the *netcoreapp3.1*, *net5.0-windows*, *net6.0-windows* or *net7.0-windows* subfolder depending on the exact framework used.
	
	In case Telerik UI for WPF is used only for the report viewer, you may reference the Telerik UI for WPF assemblies available with Telerik Reporting. They are internally unlocked for the WPF Report Viewer but can only be used with the report viewer. The Telerik UI for WPF assemblies are located in `%programfiles(x86)%\Progress\Telerik Reporting {{site.suiteversion}}\Bin\WpfViewerDependencies`. Select the *Core*, *Net5*, *Net6* or *Net7* subfolder depending on the exact framework used.

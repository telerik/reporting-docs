---
title: Telerik CLI
page_title: Telerik CLI - Installation, Setup, and Command Reference for Telerik Reporting
description: Install and use the Telerik CLI to set up the Telerik NuGet feed and update Telerik Reporting packages from the command line.
slug: reporting-telerik-cli
tags: telerik,cli,command, line,dotnet, tool,nuget,setup,install,update,reporting,getting, started,environment
published: True
position: 1
tag: new
---
...
This command installs and configures only the Telerik Reporting MCP server and targets only Visual Studio by creating or updating the `.mcp.json` file with the Reporting MCP server entry.

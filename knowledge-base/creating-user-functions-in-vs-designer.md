...
If your custom user function as a **public static** (**Public Shared** in _VB.NET_) method is a part of the current Report class:
...
1. If your custom user functions are linked from an external assembly (e.g., MyFunctions.dll), for the designer to recognize them, you will have to extend the configuration of the start application.
   ...
   For the Visual Studio Report Designer, this is the **'devenv.exe.config'** file that resides in _'C:\Program Files (x86)\Microsoft Visual Studio X.0\Common7\IDE'_ by default (it is recommended to create a backup copy before modifying it).

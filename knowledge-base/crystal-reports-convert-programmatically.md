...
Both [Visual Studio Report Designer] ({% telerikreporting/designing-reports/report-designer-tools/desktop-designers/visual-studio-report-designer/overview %}) and [Standalone Report Designer]({% telerikreporting/designing-reports/report-designer-tools/desktop-designers/standalone-report-designer/overview %}) applications allow converting a single Crystal Reports file to a Telerik Reporting report definition, but do not support converting multiple reports at once.

## Solution  

Converting multiple Crystal Report files to Telerik Reporting .trdx or .trdp files can be done programmatically. In the steps below we'll show how to build a simple console application that performs bulk conversion of all the Crystal Report files in a particular folder and output the conversion log to the console.

- **Prerequisites**

	The assemblies (both Telerik and Crystal Reports ones) that will be used for conversion process are not distributed via NuGet. They are deployed to your machine with the corresponding MSI installers. Please ensure you have installed Telerik Reporting and SAP Crystal Reports runtime on your machine.
...
- **Start the application**
  ...
  At this point the application should be runnable, given the assembly- or NuGet references are correctly configured. 
...
- **Troubleshooting**
  - "*Error: Could not load file or assembly 'CrystalDecisions.CrystalReports.Engine, Version=13.0.2000.0, Culture=neutral, PublicKeyToken=692fbea5521e1304' or one of its dependencies. The system cannot find the file specified.*"
    ...
    In case you have installed a different version of Crystal Reports runtime, you have to add binding redirect rules to the application configuration file. Check the [Converting SAP Crystal Reports]({% telerikreporting/designing-reports/converting-reports-from-other-reporting-solutions/crystal-reports-converter %}) article, section **Configuring the Report Designer to Use the Converter** for the binding redirect snippets.
  ...
  - "*System.IO.FileNotFoundException: 'Could not load file or assembly 'log4net, Version=1.2.10.0, Culture=neutral, PublicKeyToken=692fbea5521e1304' or one of its dependencies. The system cannot find the file specified.'*"
    ...
    This error indicates that the assembly **log4net.dll** assembly, which is referenced by Crystal Reports assemblies, cannot be loaded. This assembly should be installed and registered in GAC by the Crystal Reports runtime installer. In case it is missing, reinstall Crystal Reports and try again.
...
The sample application CrystalReportsConverter is uploaded to our [reporting-samples](https://github.com/telerik/reporting-samples) GitHub repo. You can download it from here: [CrystalReportsConverter](https://github.com/telerik/reporting-samples/tree/master/ApplicationExamples/CrystalReportsConverter).
...
- [Converting SAP Crystal Reports]({% telerikreporting/designing-reports/converting-reports-from-other-reporting-solutions/crystal-reports-converter %})
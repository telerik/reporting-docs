---
title: Resolving Headers Not Showing in Viewer and Font Substitution Issues
description: Learn how to fix the issue where headers do not appear in the viewer but show in the designer, and address font substitution warnings.
type: how-to
...
1. Ensure header elements, such as column headers, are within the visible bounds and not near the bottom edge.

For detailed guidance, refer to the knowledge base article: [Textbox element not showing or part of text hidden in a page section]({%slug text-cut-or-hidden-in-page-header%}).
...
1. Ensure the font is embedded in the report using the [privateFonts]({%slug telerikreporting/using-reports-in-applications/export-and-configure/configure-the-report-engine/privatefonts-element%}) configuration.

1. Use the same font across all report items to prevent inconsistencies.

Refer to these articles for further details:
* [The first instance of a report item is with a different font than the rest]({%slug font-is-incorrect-on-first-render-azure%})
* [The font of the first few report items is not consistent with the rest]({%slug inconsistent-font-of-the-first-few-report-items%})

## See Also

* [PageHeaderSection Overview](https://docs.telerik.com/reporting/api/telerik.reporting.pageheadersection)
* [Textbox element not showing or part of text hidden in a page section]({%slug text-cut-or-hidden-in-page-header%})
* [The first instance of a report item is with a different font than the rest]({%slug font-is-incorrect-on-first-render-azure%})
* [The font of the first few report items is not consistent with the rest]({%slug inconsistent-font-of-the-first-few-report-items%})
* [privateFonts Configuration]({%slug telerikreporting/using-reports-in-applications/export-and-configure/configure-the-report-engine/privatefonts-element%})

...
Angular requires a folder called `assets` to be used for serving custom files, so they could be included in the production build.

This is done via the `assets` property in the `.angular-cli.json` file (or `.angular.json` for **Angular 6+** applications).

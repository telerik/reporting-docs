...
- **Use [Fiddler Everywhere Reporter](https://www.telerik.com/fiddler/fiddler-everywhere-reporter) or another proxy tool to capture the web traffic** and check the requests, responses, and statuses.
	In the generated logs, check the failing requests from the Reporting REST Service endpoint. Usually, the Reporting service requests start with `{mainServiceUrl}/api/reports/`. Here is sample log of successfully passed requests between the HTML5 Report Viewer and the Service:

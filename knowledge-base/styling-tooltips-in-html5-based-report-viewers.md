...
- KendoUI Tooltip’s content can accept valid HTML and we provide this content using the template. The template ID is “_trv-pages-area-kendo-tooltip_” and it consists of three **div** elements – the first one is a _container_ for the other two that are used for the*title* and the _text_ respectively:
...
- The styling of each of the div elements can be controlled by its CSS class.

  The `.css` file that contains the styling is named `telerikReportViewer.css` and, similar to the report viewer template file, can be found at `{Telerik Reporting installation directory}\Html5\ReportViewer\styles` folder.
...
Using the [viewerToolTipOpening(e, args)](slug:telerikreporting/using-reports-in-applications/display-reports-in-applications/web-application/html5-report-viewer/api-reference/reportviewer/events#viewertooltipopening) event, we can make changes to the tooltip right before it is displayed.
...
- In the `onPageReady()` event, we will select all the elements that have either set tooltip _title_ or _text_ and create a new instance of the jQuery Tooltip for each one of them:

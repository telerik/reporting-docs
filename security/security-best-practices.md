...
* The declarative report definitions (_TRDP_, _TRDX_, _TRDJ_, and _TRBP_ files) describe the report layout with XML or JSON code. The Reporting Engine [unpackages](slug:telerikreporting/using-reports-in-applications/program-the-report-definition/package-report-definition) or [deserializes](slug:telerikreporting/using-reports-in-applications/program-the-report-definition/serialize-report-definition-in-xml) them to make the required [Telerik.Reporting.Report](/api/telerik.reporting.report) instance. The developer should ensure that no content is included that could result in malicious executable code, which may lead to a security breach.

...
* The declarative report definitions (_TRDP_, _TRDX_, and _TRBP_ files) describe the report layout with XML code. The Reporting Engine [unpacks the report definition package](slug:telerikreporting/using-reports-in-applications/program-the-report-definition/package-report-definition) or [deserializes the XML report definition](slug:telerikreporting/using-reports-in-applications/program-the-report-definition/serialize-report-definition-in-xml) to make the required [Telerik.Reporting.Report](/api/telerik.reporting.report) instance. The developer should ensure that no content is included that could result in malicious executable code, which may lead to a security breach.
...
* Client-side ReportSource, for example, the [HTML Report Viewer's reportSource](slug:telerikreporting/using-reports-in-applications/display-reports-in-applications/web-application/html5-report-viewer/api-reference/reportviewer/methods#reportsource).

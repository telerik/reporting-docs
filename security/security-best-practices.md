...
When providing the Public Key Token in the Assembly Qualified Name, the Reporting Engine will try to match it when searching for the DLL. It will not load an assembly with the same name and a different Public Key Token and will throw an exception instead. If the Public Key Token is omitted, the Reporting Engine will load the first DLL, which name matches the provided Assembly Qualified Name. Potentially, this may allow bad agents to replace trusted assemblies with malicious ones and constitutes a security vulnerability. The Public Key Token provides an extra security layer to your reports.

...
> important Note that **MS Office PowerPoint Viewer 2007** doesn't support all the OpenXML features and in some cases fails to open the exported **.pptx** files.
...
> important Using **Microsoft PowerPoint 2019 or newer** is required for the exported documents with to open properly.
...
> important The [`UseSvgImages`](slug:telerikreporting/using-reports-in-applications/export-and-configure/configure-the-export-formats/powerpoint-device-information-settings) setting requires [DocumentFormat.OpenXml 3+](https://www.nuget.org/packages/documentformat.openxml). If an older version is loaded, the setting will be evaluated to **false**.
